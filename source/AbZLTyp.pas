...
begin
  ...
  FZLHeader.FLG := AbToUInt8(FZLHeader.FLG and not AB_ZL_PRESET_DICT); { no preset dictionary}
...
begin
  Result := AbToUInt8(FZLHeader.CMF shr 4);
end;

function TAbZLItem.GetCompressionLevel: Byte;
begin
  Result := AbToUInt8(FZLHeader.FLG shr 6);
end;
...
var
  zlh : Word;
begin
  FZLHeader.FLG := AbToUInt8(FZLHeader.FLG and not AB_ZL_FCHECK_MASK);
  zlh := AbToUInt16((FZLHeader.CMF * 256) + FZLHeader.FLG);
...
begin
  FZLHeader.CMF := AbToUInt8(FZLHeader.CMF and not AB_ZL_CINFO_MASK);
  FZLHeader.CMF := AbToUInt8(FZLHeader.CMF or (Value shl 4)); { shift value and add to bit field }
end;
...
begin
  ...
  FZLHeader.FLG := AbToUInt8(FZLHeader.FLG and not AB_ZL_FLEVEL_MASK);
  FZLHeader.FLG := AbToUInt8(FZLHeader.FLG or (Temp shl 6)); { shift value and add to bit field }
end;

procedure TAbZLItem.SetCompressionMethod(Value: Byte);
begin
  if Value > AB_ZL_CM_MASK then Value := AbToUInt8((Value shl 4) shr 4);
  FZLHeader.CMF := AbToUInt8(FZLHeader.CMF and not AB_ZL_CM_MASK);
