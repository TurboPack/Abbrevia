...
begin
  ...
  Result := AbToUInt8(aCh xor ((Temp * (Temp xor 1)) shr 8));
  ...
  FState[2] := AbUpdateCrc32(AbToUInt8(FState[1] shr 24), FState[2]);
end;
{--------}
...
begin
  ...
  for i := 0 to pred(aCount) do begin
    ...
    WorkState[2] := AbUpdateCrc32(AbToUInt8(WorkState[1] shr 24), WorkState[2]);
...
begin
  ...
  for i := 0 to 11 do begin
    ...
    {calculate the next decoded byte (uses inlined decrypt_byte)}
    Temp := (FState[2] and $FFFF) or 2;
    WorkHeader[i] := AbToUInt8(aHeader[i] xor ((Temp * (Temp xor 1)) shr 8));
...
begin
  ...
  for i := 0 to 9 do begin
    ...
    WorkHeader[i] := AbToUInt8(Ch xor Temp);
  end;
  ...
  for i := 0 to 9 do begin
    ...
    {set the current byte of the header}
    WorkHeader[i] := AbToUInt8(WorkHeader[i] xor Temp);
  end;
  ...
  FState[1] := (FState[1] * MagicNumber) + 1;
  FState[2] := AbUpdateCrc32(AbToUInt8(FState[1] shr 24), FState[2]);
  WorkHeader[10] := AbToUInt8(Ch xor Temp);
  ...
  FState[1] := (FState[1] * MagicNumber) + 1;
  FState[2] := AbUpdateCrc32(AbToUInt8(FState[1] shr 24), FState[2]);
  WorkHeader[11] := AbToUInt8(Ch xor Temp);
